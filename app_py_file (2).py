...
def calculate_shuttle_optimization(df, buffer_factor=1.2):
    """Calculate optimized shuttle counts with buffer logic"""
    # Group by time block and day of week, taking the AVERAGE across all dates
    # This gives us the typical demand for each time block on each day of week
    grouped = df.groupby(['time_block', 'day_of_week'])['passenger_count'].mean().reset_index()
    
    # For planning purposes, we might want to use a higher percentile (e.g., 75th percentile)
    # to ensure we can handle above-average days
    ...
    # Use the 75th percentile for shuttle planning (more conservative)
    grouped['planning_passengers'] = grouped['passenger_count_75th']
    
    # Assume shuttle capacity of 12 passengers
    shuttle_capacity = 12
    
    # Calculate required shuttles with buffer
    grouped['required_shuttles'] = np.ceil(grouped['planning_passengers'] * buffer_factor / shuttle_capacity)
    grouped['required_shuttles'] = grouped['required_shuttles'].astype(int)
    
    # Ensure minimum of 1 shuttle if there are any passengers
    grouped['required_shuttles'] = np.where(
        grouped['planning_passengers'] > 0, 
        np.maximum(grouped['required_shuttles'], 1), 
        0
    )
...
if st.session_state.data is not None:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📈 Analytics Dashboard", "🎯 Route Optimization", "💰 Cost Analysis", "📋 Data Summary"])
    ...
    with tab2:
        ...
        st.info("🎯 **Planning Methodology:** Shuttle counts based on 75th percentile demand + buffer factor to handle above-average days while avoiding over-provisioning.")
        ...
        # Show planning vs average comparison
        col1, col2 = st.columns(2)
        ...
        with col2:
            # 75th percentile demand table
            p75_demand = df.groupby(['time_block', 'day_of_week'])['passenger_count'].quantile(0.75).reset_index()
            p75_pivot = p75_demand.pivot(index='time_block', columns='day_of_week', values='passenger_count').fillna(0)
            p75_pivot = p75_pivot.reindex(columns=day_order, fill_value=0).round(1)
            
            st.write("**75th Percentile Demand (Planning Basis)**")
            st.dataframe(p75_pivot, height=300)
        ...
        with col1:
            st.metric("Average Daily Shuttles", f"{avg_daily_shuttles:.0f}")
        with col2:
            peak_day = total_daily_shuttles.idxmax()
            peak_shuttles = total_daily_shuttles.max()
            st.metric("Peak Day", f"{peak_day}: {peak_shuttles} shuttles")
        with col3:
            min_day = total_daily_shuttles.idxmin()
            min_shuttles = total_daily_shuttles.min()
            st.metric("Light Day", f"{min_day}: {min_shuttles} shuttles")
        
        st.info(f"**Optimization Parameters:** Buffer Factor: {buffer_factor:.1f}x | Shuttle Capacity: 12 passengers | Planning Basis: 75th percentile demand | Time Block: 30 minutes")

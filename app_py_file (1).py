...
def calculate_shuttle_optimization(df, buffer_factor=1.2):
    ...
    # Group by time block and day of week
    grouped = df.groupby(['time_block', 'day_of_week'])['passenger_count'].sum().reset_index()
    
    # Assume shuttle capacity of 12 passengers
    shuttle_capacity = 12
    
    # Calculate required shuttles with buffer
    grouped['required_shuttles'] = np.ceil(grouped['passenger_count'] * buffer_factor / shuttle_capacity)
...
if st.session_state.data is not None:
    ...
    with tab2:
        ...
        st.info(f"**Optimization Parameters:** Buffer Factor: {buffer_factor:.1f}x | Shuttle Capacity: 12 passengers | Time Block: 30 minutes")
